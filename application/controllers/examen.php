...
class Examen extends CI_Controller {
    ...
    public function listar_indicadores() {
        ...
        $isPerfil = $this->input->post('b_isPerfil');
...
//        }
        ...
        $inidicadores = $this->Registerexamen_model->listar_indicadores_jqgrid($page, $limit, $sidx, $sord, $idSearch, $isPerfil);
        echo $inidicadores;
    }
...
//        }
    ...
    public function listar_perfiles() {
        ...
        $sidx = $this->input->post('sidx');
        $sord = $this->input->post('sord');
        $idSearch = $this->input->post('i_idperfil');
    ...
    public function listar_tipoexamen() {
        ...
        try {
            ...
            $a_examenes = $this->db->select('exa_d.exad_id, exa_d.exa_id,exa_d.exad_nombre,t_ex.tex_antibiograma')
                    ->from('examen_detalle AS exa_d')
                    //->like('ind.med_fullname', $dato, 'both')
                    ->join('tipos_examen t_ex', 'exa_d.tex_id = t_ex.tex_id', 'inner')
    ...
    public function listar_indicadoresByExamen() {
        ...
        try {
            ...
            $a_indicadores = $this->db->select('d_iex.indd_id,ind.ind_id AS idIndicador,ind.ind_descripcion,d_iex.indd_valor_resultante2,d_iex.indd_valor_resultante,ind.ind_unidad_medida,ind.ind_rango_referencial')
...
//
    ...
    public function listar_hematograma() {
        $aResponse = $this->json_array_return;
        try {
            $dato = $this->input->post('idDetExamen');

            $a_hematograma = $this->db->select('*')
                    ->from('detalle_indicador_especial AS d_ies')
                    //->join('indicadores ind', 'd_iex.ind_id = ind.ind_id', 'inner')
                    //->like('ind.med_fullname', $dato, 'both')
                    ->where('d_ies.indd_id', $dato)
                    ->get()
                    ->result_array();
//            echo count($a_hematograma);
//            print_r($this->db->last_query());
//           die();

            $aResponse['count'] = count($a_hematograma);
            $aResponse['data'] = $a_hematograma;
        } catch (Exception $exc) {
            $aResponse['code'] = self::CODE_ERROR;
            $aResponse['msg'] = $exc->getMessage();
        }
    ...
    public function guardar_resultado_indicador_especial() {
        ...
        try {
            ...
            $i_input_indd_id = trim($this->input->post('input_indd_id'));
            ...
            if ($i_indicador_resultado_id != 0) {
                ...
                if ($i_input_indd_id == 0) {
                    $this->db->set($data_de);
                    $this->db->insert('detalle_indicador_especial');
                } else {
                    $this->Resultados_examen_model->editar_resultado_indicador_especial($i_input_indd_id, $data);
                }
            } else {
                //$ver = $this->paciente_model->agregar_paciente($data);
                echo 'No existe indocador Asociado.';
...
//            die();
            ...
            if ($this->db->trans_status() === FALSE) {
                $this->db->trans_rollback();
                $response['codExamen'] = 0;
            } else {
                $this->db->trans_commit();
                $response['codExamen'] = $cod_examen;
            }
        } catch (Exception $e) {
            echo $e->getMessage();
            throw $e;
    ...
    public function printDataToExport($idSearch) {
        ...
        //$tipo_examen = $this->getTipoExamenPrint('null');
//


        foreach ($tipo_examen as $key => $value) {
//            echo 'ee';
//            echo 'ff'.$value->tex_id;
//            die();
//            $id_tipo_examen = $value[$key]['exad_id'];
    ...
    public function printDataToExport_mitad($idSearch) {
        ...
        $this->load->view('examen/exportar_mitad', $data);
    }
...
//                $sub_examenes->where('tipos_examen.tex_idPadre', NULL);
            ...
            $options = $this->db->select('*')
                    ...
                    ->order_by("tipos_examen.tex_order", "asc")
                    ->get()
                    ->result_array();
...
//            die();
    ...
    public function getTipoExamenPrint($key) {
        // $key = $this->input->post('key');
        $adic_where_key = "";
        $adic_where_parent = "";
        //print_r($key); die();
